...
class ProductController extends Controller {
  async addNewProduct(req, res) {
    ...
    // فایل
    if (!req.file) {
      throw createHttpError.BadRequest("تصویر محصول الزامی است");
    }
   
    const imageLink = `${req.uploadFolder}/${req.file.filename}`.replace(
      /\\/g,
      "/"
    );
    const product = await ProductModel.create({
      ...
      description,
      slug,
      imageLink: `${req.protocol}://${req.get("host")}/${imageLink}`,
      brand,
      tags,
      category,
      price,
