...
import { userRols, userStatus } from "../utils/resourse";

export default function UsersTable({ tableHead, tableBody }) {
  return (
    <div className="flex flex-col gap-y-2.5 border border-secondary-150 rounded-lg p-2.5">
      ...
      <div className="flex flex-col gap-y-2.5">
        {tableBody?.map((tB) => {
          const { _id: userId, name, phoneNumber, role, createdAt: date } = tB;
          const statusStyle = userStatus[tB.isActive];
          const userRole = userRols[role];
          return (
